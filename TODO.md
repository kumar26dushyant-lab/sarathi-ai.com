...
- **Claim raise forms — align data (NEXT, careful, endpoint-by-endpoint w/ before-after):** **Complainant** block (name, mobile+email MANDATORY, role) with in-form "why we need it"; **Insured/patient** (name, mobile optional). Comms use complainant contact. Forms: branch raise, subscriber (dashboard+start), ops my-claims, ₹499 review, review-request. Then escalation routing (complainant silent → referrer → ops, all channels).
