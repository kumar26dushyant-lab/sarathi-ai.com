...
- **WA orchestrator ✅ LIVE (in-session guided flow).** `biz_nidaan_wa_orchestrator.py`: claimant messages number → matched to claim by phone → greet + ask next pending doc → inbound doc → download → PDF → Gemini right-doc/quality gate → save + mark checklist → ask next/complete; recorded on claim timeline. Manual start: drawer "💬 Start WhatsApp collection" + `POST /claims/{id}/wa/start`. Webhook verified.
  ...
  - **Templates draft ✅ READY for founder** — `WA_TEMPLATES_TO_SUBMIT.md` has 6 submit-ready bilingual templates (np_claim_registered / np_welcome / np_intro_value / np_payment_thanks / np_payment_failed / np_doc_reminder) with categories + samples. **OWNER ACTION:** paste into Meta → Message templates; after approval fill `JOURNEY_TEMPLATES` in `biz_nidaan_wa_orchestrator.py` → cold-start sends go live.
  - **REMAINING:** (1) **payment-event → complainant WA** (`thank_you_payment`/`payment_failed`) needs de-dup design — `on_funnel_paid` already WA-confirms the SUBSCRIBER phone; complainant≠subscriber (branch/ops) claims are uncovered. Wire at claim-linked payment callsites with a subscriber-phone-collision guard. (2) reminder loop (Phase 2, template-gated). (3) escalation routing (complainant silent → referrer → ops).
