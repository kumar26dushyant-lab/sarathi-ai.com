...
**Last updated:** 2026-09-17 (branch login outage traced to Workspace anti-spoofing and fixed; WhatsApp added as the second way in; App Health + the watchdog now see every login system and say why one is broken — 37 tests)
...
- 🟢 **Next for this area:** submit an **authentication template** to Meta so a WhatsApp login code can reach a branch that has NOT written to us in the last 24 hours — today the fallback only works inside that window.
