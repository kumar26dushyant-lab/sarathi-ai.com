...
- 🔴 **Narrow `health.subsystem` to super_admins?** Infra alerts go to all **14** super/sub-super admins, but re-checking a mailbox app password is realistically only actionable by a super_admin. Narrowing it the way `payment.*` was narrowed is the founder's call — narrowing an infra alarm can hide a real outage from whoever would have caught it.
